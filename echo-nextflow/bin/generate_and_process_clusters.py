    ...
    with open(fasta_file, "w") as fasta_file:
        for _, row in df.iterrows():
            ...
            if sequence is not None:
                ...
                print(f"Warning: No sequence found for header {header}")
...
def process_clusters(output_dir, num_relatives):
    """
    Process the cluster parquet file in output_dir.
    Supports either 'clusters.parquet' or 'clusters_batch_*.parquet'.
    """
    # Find parquet file in output_dir
    ...
    print(f"Using cluster file: {output_cluster_name}")

    # Connect to DuckDB and load the table
    con = duckdb.connect()
    con.execute(
        f"CREATE OR REPLACE TABLE clusters AS SELECT * FROM read_parquet('{output_cluster_name}')"
    )
