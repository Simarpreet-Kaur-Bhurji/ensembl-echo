...
def load_cluster_dict(cluster_json_path):
    """Load cluster dictionary from JSON file."""
...
def cluster_distribution_by_taxids(cluster_log_path, less_than_required_log_path, output_dir):
    # Load data
    cluster_df = load_tsv(cluster_log_path)[['Cluster_ID', 'Num_Unique_TaxIDs']]
    less_than_df = load_tsv(less_than_required_log_path)[['Cluster_ID', 'Total_TaxIDs']]
    
    # Rename column to match cluster_df
    less_than_df = less_than_df.rename(columns={'Total_TaxIDs': 'Num_Unique_TaxIDs'})
    
    combined_df = pd.concat([cluster_df, less_than_df], ignore_index=True)
    combined_df['Num_Unique_TaxIDs'] = combined_df['Num_Unique_TaxIDs'].astype(int)
    
    # Group by Num_Unique_TaxIDs
    distribution_df = combined_df.groupby('Num_Unique_TaxIDs').size().reset_index(name='Num_Clusters')
    distribution_df = distribution_df.sort_values('Num_Unique_TaxIDs')
    ...
    fig = px.line(
        ...
        markers=True,
        labels={
            "Num_Unique_TaxIDs": "Unique Tax IDs per cluster",
    ...
    fig.update_traces(
        hovertemplate="Unique Tax IDs: %{x}<br>Clusters: %{y:.0f}"
    )
    
    # Add label in top-right corner
    max_taxids = distribution_df['Num_Unique_TaxIDs'].max()
    fig.add_annotation(
...
def plot_proteins_per_cluster(cluster_json_path, output_dir):
    # Load clusters dict
    clusters_dict = load_cluster_dict(cluster_json_path)
    
    # Compute number of proteins per cluster
    cluster_sizes = {k: len(v) for k, v in clusters_dict.items()}
    df = pd.DataFrame(list(cluster_sizes.items()), columns=['Cluster_ID', 'Num_Proteins'])
    ...
    fig.update_traces(
        hovertemplate="Proteins: %{x}<br>Clusters: %{text}",
        text=dist['Num_Clusters']
    )
...
def cluster_summary(cluster_json_path, less_than_log, closest_rel_log, output_dir):
    ...
    num_closest_relatives_clusters = total_clusters - (total_singletons + num_fewer_taxid_clusters)
    ...
    with open(summary_file, mode) as f:
        f.write("\n")
        f.write(f"Total number of clusters obatined from input fasta: {total_clusters}\n")
        f.write(f"Singleton clusters (clusters with only one protein): {total_singletons}\n")
        f.write(f"Number of clusters with fewer than the required tax IDs: {num_fewer_taxid_clusters}\n")
        f.write(f"Closest relatives clusters (clusters with required tax IDs or more): {num_closest_relatives_clusters}\n")
...
def echo_pipeline_summary(output_dir, input_fasta_file):
    ...
    with open(summary_file, "w") as f:
        ...
        for query, stats in retention_stats.items():
            f.write(f"  {query} : {stats['Retained']} sequences "
                    f"({stats['Percent Retained']:.2f}% retained, "
                    f"{stats['Percent Discarded']:.2f}% discarded)\n")
        f.write("Note: Each query_all_relatives.fa file includes sequences from clusters_with_fewer_tax_ids.fa.\n\n")
        f.write(f"Total sequences in all_queries_combined_relatives.fa: {all_queries_combined_count}\n")
        f.write("Note: This file combines all query_all_relatives.fa files and includes sequences from clusters_with_fewer_tax_ids.fa.\n")
...
def get_diagnostic_stats_and_plots(input_fasta_file, output_dir):
    ...
    cluster_json_path = os.path.join(output_dir, "cluster_dict.json") if "cluster_dict.json" in files else None
    closest_rel_log = os.path.join(output_dir, "closest_relatives_log.tsv") if "closest_relatives_log.tsv" in files else None
    less_than_log = os.path.join(output_dir, "clusters_with_fewer_taxids_summary.tsv") if "clusters_with_fewer_taxids_summary.tsv" in files else None
    singleton_log = os.path.join(output_dir, "singleton_cluster_summary.tsv") if "singleton_cluster_summary.tsv" in files else None
    ...
    plot_proteins_per_cluster(cluster_json_path, output_dir)

    # Distribution of clusters by unique tax IDs
    cluster_by_taxids = cluster_distribution_by_taxids(closest_rel_log, less_than_log, output_dir)

    # Cluster summary (print stats and write to file)ls
    
    cluster_summary(cluster_json_path, less_than_log, closest_rel_log, output_dir)
